...
class ProductsController < ApplicationController
  ...
  def product
    @product = ProductFinder.search(deleted: check_if_admin? ? nil : false, id: params[:id]).first
  end

  def pagination
    all_products = check_if_admin? ? Product.order(:id) : ProductFinder.search(deleted: false).order(:id)
